...
echo 'protein' 'system' | trjconv_avx -f $(basename $XTC '.xtc')_noPBCWhole_noJump_skip$SKIP.xtc -o $(basename $XTC '.xtc')_noPBCWhole_noJump_Center_skip$SKIP.xtc -pbc atom -center -ur compact -s $TPR

echo 'protein' 'system' | trjconv_avx -f $(basename $XTC '.xtc')_noPBCWhole_noJump_Center_skip$SKIP.xtc -o $(basename $XTC '.xtc')_noPBCComplete_skip$SKIP.xtc -fit progressive -s $TPR
