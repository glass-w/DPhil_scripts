...
#				tmp_lipid_centered = coords_center_in_box(coords_lip, coord_CoG[p_index], box_size)
				...
				vector_list[p_index,nb_frame,:] = rotate_coord(coord_pt, rot)
				#print ('coord_pt, angle_2D_between(coord_pt, [0,1,0]), rot, vector_list[p_index,nb_frame,:]', coord_pt, angle_2D_between(coord_pt, [0,1,0]), rot, vector_list[p_index,nb_frame,:])

				# TO CHECK THE VALUES IN HARD
				neighbours_prot[p_index][nb_frame] = rotate_coord(tmp_prot_neighbour,rot)
#				neighbours_prot[p_index][nb_frame] = rotate_coord(tmp_lipid_centered,rot)
	...
	plot_density_array(array_bin, xedges, yedges, -cutoff, cutoff, -cutoff, cutoff, 80, ax_vector_prot,'binary')
